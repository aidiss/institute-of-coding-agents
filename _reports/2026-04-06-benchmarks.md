...
| 7 | Sonar Foundation Agent (Sonar/Anthropic) | SWE-bench Verified | 79.2% | Feb 2026 (self-reported) |
| 8 | Qwen3.6 Plus (Alibaba) | SWE-bench Verified | 78.8% | ~Mar 2026 (self-reported) |
| 9 | Gemini 3 Flash (Google) | SWE-bench Verified | 78.0% | ~Mar 2026 (self-reported) |
| 9 | MiMo-V2-Pro (Xiaomi) | SWE-bench Verified | 78.0% | ~Mar 2026 (self-reported) |

> **All 80+ entries are self-reported. Zero are independently verified.** The official swebench.com "bash-only" scaffold shows Claude 4.5 Opus at 76.8% — a ~4-point gap vs. vendor-reported numbers from proprietary scaffolds. Sonar Foundation Agent (entry #7) uses Claude Opus 4.5 as its underlying model and scored 79.2% on SWE-bench Verified plus 52.62% on SWE-bench Full — the agent scaffold claims credit, not a new base model. Per SWE-bench's own cheating detection, all frontier models tested by OpenAI reproduced verbatim gold patches including variable names not present in problem descriptions.
...
> Custom-scaffold self-reported results (non-standardized): OpenAI claims GPT-5.4 at ~57.7% and GPT-5.3-Codex at ~56.8% with proprietary harness — a ~15-point lift over SEAL scores purely from scaffold design. Claude Opus 4.5 similarly ranges from 45.9% (SEAL) to ~55% (custom scaffold). **Labs reporting with proprietary scaffolding are not directly comparable to SEAL entries.**
...
> Step-3.5-Flash at $0.14/problem is a notable cost-efficiency outlier — competitive accuracy at ~8x lower cost than Anthropic models. SWE-rebench explicitly flags labs that report pass@N as pass@1 without disclosure.
...
> All entries self-reported; zero independently verified. ForgeCode is the leading scaffold. Top scores cluster near 81%, suggesting near-saturation for frontier agents on this version. Separate model-only (no harness) scores diverge significantly — llm-stats model-only leaderboard shows GPT-5.3 Codex at 77.3% vs. 78.4% under SageAgent scaffold.

---

### LiveCodeBench (Top 10)

Continuous anti-contamination benchmark sourcing problems from LeetCode, Codeforces, and AtCoder published after each model's training cutoff. Pass@1. Date-windowed per model to prevent contamination. Source: livecodebench.github.io / llm-stats.com, Apr 2026.
...
> **Note:** Alternative trackers (pricepertoken.com, Apr 3, 2026) report Gemini 3 Pro Preview at 91.7% on a different evaluation window — LiveCodeBench scores are window-dependent by design; cross-tracker comparisons require identical date-window configuration to be meaningful. DeepSeek, xAI, and Meituan dominate the standardized ranking; Anthropic and OpenAI flagship models are absent from top 10.
...
### Coding Arena (Blind Human Preference — Top 3)

Blind head-to-head arena votes; users compare code outputs without knowing which model generated them. Source: arena.ai/leaderboard/code, Apr 2026.

| Rank | Agent | Benchmark | Score | Date |
|------|-------|-----------|-------|------|
| 1 | Claude Sonnet 4.6 (Anthropic) | Coding Arena | 1062 (ELO) | Apr 2026 |
| 2 | Boba | Coding Arena | 1059 (ELO) | Apr 2026 |
| 3 | Gemini 3 Flash (Google) | Coding Arena | 780 (ELO) | Apr 2026 |

> Based on 1,324 blind votes. Human preference rankings diverge from automated benchmark rankings — Claude Sonnet 4.6 tops human preference but sits mid-table on SWE-bench Verified.
...
|-----------|-----------|-------------|
| **SWE-bench Pro** (GA) | Scale AI, Sep 2025 | 1,865 multi-language tasks (Python, Go, TS, JS); copyleft/private repos to reduce contamination; endorsed by OpenAI as Verified replacement |
...
| **Terminal-Bench 2.0** | tbench.ai, Nov 2025 | Major revision: 89 tasks, removed step limits, added auxiliary interfaces with function signatures; ICLR 2026 accepted paper |
...
### Sonar Foundation Agent Claims Top Spot (Feb 2026)
Sonar announced its Sonar Foundation Agent reached #1 on the unfiltered SWE-bench Verified leaderboard with 79.2% (SWE-bench Verified) and 52.62% (SWE-bench Full), using Claude Opus 4.5 as the base model. The agent is LLM-agnostic but tuned for static analysis and code quality workflows. **The result is self-reported; the agent is not commercially available.** It has since been overtaken by Claude Opus 4.5 and 4.6 direct entries at 80.9% and 80.8%.
...
### LiveCodeBench Window-Dependence Problem
Different score trackers now report substantially different LiveCodeBench figures for the same models depending on which date window is used. Alternative trackers report Gemini 3 Pro Preview as high as 91.7% (Apr 3 window) vs. ~80% on the standard window. The benchmark remains structurally sound but cross-source comparisons are increasingly unreliable without window disclosure.

### MIT Technology Review: "AI Benchmarks Are Broken" (Mar 31, 2026)
MIT Technology Review published an analysis arguing that of 15 major benchmarks in active use, only 4 reliably predict production outcomes. Key critique: most benchmarks measure academic performance on fixed tasks. Proposed remedies: contamination-resistant rolling datasets, metric unification, transparent governance, and auditable fairness guarantees.

### Industry-Wide Self-Reporting Problem Unresolved
As of April 6, 2026: no major coding benchmark (SWE-bench Verified, SWE-bench Pro SEAL, Terminal-Bench, LiveCodeBench, Aider Polyglot) has independently verified results. Every score on every leaderboard is self-submitted by the model developer or their agent. There are no industry standards for contamination detection and no enforcement mechanisms.
...
- [SWE-Bench Verified Leaderboard March 2026 — Marco Patzelt](https://www.marc0.dev/en/leaderboard)
...
- [SWE-Bench Pro Leaderboard (2026): Why 46% Beats 81% — morphllm.com](https://www.morphllm.com/swe-bench-pro)
- [SWE-bench February 2026 leaderboard update — Simon Willison](https://simonwillison.net/2026/Feb/19/swe-bench/)
- [Sonar Claims Top Spot on SWE-bench leaderboard — Sonar](https://www.sonarsource.com/company/press-releases/sonar-claims-top-spot-on-swe-bench-leaderboard/)
- [Terminal-Bench 2.0 Benchmark 2026: 100 LLM Scores Ranked — BenchLM.ai](https://benchlm.ai/benchmarks/terminalBench2)
...
- [Best AI for Coding 2026 — llm-stats.com](https://llm-stats.com/leaderboards/best-ai-for-coding)
